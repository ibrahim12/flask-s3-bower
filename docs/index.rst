...
Using Flask-S3-Bower
==============

Flask-S3-Bower is incredibly simple to use. In order to start serving your
Flask application's assets from Amazon S3, the first thing to do is let
Flask-S3-Bower know about your :class:`flask.Flask` application object.
...
.. code-block:: python
    ...
    s3bower = FlaskS3Bower()

    def start_app():
        app = Flask(__name__)
        s3bower.init_app(app)
        return app
...
.. code-block:: python
    ...
    >>> flask_s3_bower.create_all(app)
    >>>

Flask-S3-Bower will proceed to walk through your application's static assets,
...
Within your bucket on S3, Flask-S3-Bower replicates the static file hierarchy
defined in your application object and any registered blueprints. URLs
generated by Flask-S3-Bower will look like the following:
...
or If you choose bower via ``USE_BOWER=True`` in settings urls become,
``/bower/foo/style.css``
...
Flask-S3-Bower Options
----------------

Within your Flask application's settings you can provide the following
settings to control the behaviour of Flask-S3-Bower. None of the settings are
...
`USE_BOWER`                 By default `USE_BOWER` is set to False. 
                            If `DEBUG` is set to False and `USE_S3` is enabled `USE_BOWER` is ignored.
                            To Enable bower support, make `USE_BOWER=True` and make ``USE_S3=False`` or ``DEBUG=True`` 
...
Flask-S3-Bower is a very simple extension. The few exposed objects, methods
and functions are as follows.

The FlaskS3 Object
------------------
.. autoclass:: FlaskS3Bower
